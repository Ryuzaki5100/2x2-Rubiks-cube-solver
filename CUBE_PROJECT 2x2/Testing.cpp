...
map<int,string> OPSIDE={{-2,"RRRDRDDDRRRDRDDD"},{1,"RRRDDDRDRRRDDDRD"},{3,""}};
...
int main()
{
    ...
    // Print_Res();
    position=Perform_first_layer(position,7);
    position=Perform_first_layer(position,6);
    position=Perform_first_layer(position,5);
    for(int i=0;i<8;i++)
    {
        cout<<"{ ";
        for(int j=0;j<2;j++)
