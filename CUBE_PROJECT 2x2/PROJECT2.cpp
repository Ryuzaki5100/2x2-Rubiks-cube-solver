...
map<int, string> ADJUST = {{4, "D"}, {5, "DD"}, {6, "DDD"}, {7, ""}};
...
vector<vector<int>> Perform_first_layer(vector<vector<int>> position, int i)
{
    ...
    else
    {
        ...
        if (position[i][1] == -3 && i == 7)
        {
            position = Execute(position, ADJUST[temp]);
            return position;
        }
        position = Execute(position, FrontCase[position[i][0]]);
        position = Execute(position, FrontCase[position[i][0]]);
...
{
    vector<int> stats, pos;
    for (int i = 0; i < 4; i++)
        pos.push_back(position[i][0]);
    pos.push_back(pos[0]);
    for (int i = 0; i < pos.size() - 1; i++)
        if ((pos[i + 1] - pos[i] + 4) % 4 == 1)
            stats.push_back(i);
    return stats;
}
vector<vector<int>> Perform_Headline(vector<vector<int>> position)
{
    ...
    if (stats.size() == 1)
    {
        position = Execute(position, HEADLINE[position[stats[0]][0]] + "RRRFRRRBBRFFFRRRBBRR");
        return position;
    }
    else if (stats.size() == 0)
    {
        position = Execute(position, "FRUUURRRUUURURRRFFFRURRRUUURRRFRFFF");
        return position;
    }
    else if (stats.size() == 3 || stats.size() == 4)
    {
...
int main()
{
    vector<vector<int>> position = {{1, 3}, {7, -3}, {5, 1}, {0, 3}, {3, -2}, {4, -3}, {2, 1}, {6, -3}};
    for (int i = 0; i < 8; i++)
        for (int j = 0; j < 2; j++)
            cin >> position[i][j];
    for (int i = 0; i < 8; i++)
