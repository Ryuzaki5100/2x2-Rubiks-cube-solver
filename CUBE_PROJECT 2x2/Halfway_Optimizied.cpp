...
string func()
{
    ...
    vector<string> op = {"R", "U", "F", "B", "L", "D"};
    q.push({pos, ""});
    while (q.front().second.length() < 8)
    {
        auto x = q.front();
        q.pop();
        for (auto i : op)
        {
            ...
            if (sol1.count(conv(temp)) && (result == "" || alg(result).first.size() > alg(sol1[conv(temp)] + revAlgo(x.second + i)).first.size()))
                result = sol1[conv(temp)] + revAlgo(x.second + i);
...
int main()
{
    ...
    vector<vector<int>> solvePos = {{0, 3}, {1, 3}, {2, 3}, {3, 3}, {4, -3}, {5, -3}, {6, -3}, {7, -3}};
    // for (int i = 0; i < 8; i++)
    //     for (int j = 0; j < 2; j++)
    //         cin >> position[i][j];
    ...
    vector<string> res = alg(func()).first;
    for (auto i : res)
        cout << i << " ";
